...
import { Menu, Plus, User as UserIcon, LogOut, Settings } from 'lucide-react';
...
export function StudioHeader({ title, subtitle, onMenuClick, action }: StudioHeaderProps) {
  ...
  return (
    <motion.div 
      className="sticky top-0 z-40 bg-bg-elevated border-b border-border px-4 py-3 flex items-center gap-3"
    ...
    >
      <button
        onClick={onMenuClick}
        className="lg:hidden p-2 hover:bg-bg-secondary rounded-lg transition-colors text-text-secondary"
      >
        <Menu size={24} />
      </button>
